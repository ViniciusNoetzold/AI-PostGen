...
const logColor = (type: LogType) =>
  ...
  type === 'warn' ? 'text-yellow-600 dark:text-yellow-500' :
...
export function VideoOutput({ appState, videoUrl, logs }: VideoOutputProps) {
  ...
  return (
    <div className={`w-full relative flex items-center justify-center transition-all ${
      hasError 
        ? 'min-h-[220px] h-auto py-6 overflow-y-auto border border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800/50 rounded-lg' 
        ...
        <div className="flex flex-col items-center text-center px-6 w-full max-w-md">
          ...
          {lastLog?.type === 'error' && (
            <div className="mt-4 w-full flex items-start gap-3 rounded-md border border-red-200 dark:border-red-900/50 bg-red-50 dark:bg-red-900/20 px-4 py-3 text-left">
              ...
              <p className="font-mono text-xs leading-relaxed text-red-700 dark:text-red-200 break-words">
